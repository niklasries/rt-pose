...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--image", type=str, required=True, help="Path to image or URL to image")
    parser.add_argument("-s", "--save-path", type=str, required=True, help="Path to save directory")
    parser.add_argument(
        "-o",
        "--detection",
        type=str,
    ...
    logger.info(f"Saving output to `{args.save_path}`...")
    dst_dir = os.path.dirname(args.save_path)
    if dst_dir:
        os.makedirs(dst_dir, exist_ok=True)
    Image.fromarray(annotated_image).save(args.save_path)
